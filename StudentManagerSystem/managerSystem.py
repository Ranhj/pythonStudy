...
class StudentManager(object):
    ...
    def run(self):
        ...
        while True:
            ...
            self.show_menu()
            # 3.用户输入目标功能序号
            menu_num = int(input('请输入您要使用的功能序号：'))
    ...
    def add_student(self):
        ...
        gender = input('请输入要添加的学员性别：')
        tel = input('请输入要添加的学员手机号：')
    ...
    def modify_student(self):
        ...
        # 2.遍历列表数据  如果用户输入的学员存在则修改姓名、性别、手机号等数据，否则提示学员不存在
        for i in self.student_list:
            if modify_name == i.name:
                i.name = input('请输入学员姓名：')
                i.name = input('请输入学员性别：')
                i.name = input('请输入学员手机号：')
    ...
    def search_student(self):
        ...
        # 2.遍历数据列表  如果用户输入的目标学员存在则打印学员信息，否则提示学员不存在
        for i in self.student_list:
            if search_name == i.name:
                print(f'您查询的学员姓名是{i.name}, 性别是{i.gender}, 手机号是{i.tel}')
                break
            else:
                print('查无此人')
    ...
    def load_student(self):
        ...
        try:
            ...
            # 2.读取数据：文件读取出的数据是字符串换原列表类型： [{}]转换[学员对象]
            data = f.read() # 字符串
            new_list = eval(data)
